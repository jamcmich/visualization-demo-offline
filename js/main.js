...
function setupGUI(config) {
	...
	if (config.logo.file) {
		...
		logo = "<img style='width:250px' src=\"" + config.logo.file +"\"";
...
function configSigmaElements(config) {
    ...
    var colleges = [
        ...
        'Unlisted',
        'Design and the Arts, Herberger Institute for (HIDA)',
        'Teachers College, Mary Lou Fulton (MLFTC)',
        'Future of Innovation in Society, School for the (SFIS)',
        'Knowledge Enterprise (KE)',
        'Executive Vice President and Provost, Office of the (EVPP)',
        'Business and Finance, Office of (OBF)',
        ...
        'Executive Administration, Office of (EA)',
        ...
        'Global Futures Laboratory (GFL)',
        'University College (UC)',
        'Global Management, Thunderbird School of (TSGM)'
    ];
    var a = [],
        b,x=1;
		for (b in sigInst.clusters) a.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;border:1px solid #fff;background:' + b + ';display:inline-block"></div> ' + (colleges[x++]) + ' (' + sigInst.clusters[b].length + ' members)</a></div>');
    // a.sort();
